...
export const RoleSelectionPage: React.FC = () => {
  ...
  const { currentRole, setRole, continueAsDemo, isAuthenticated, user, logout } = useAuth();
  ...
  const handleSelectRole = (role: UserRole) => {
    // If authenticated, role is locked and cannot be changed in-session
    if (isAuthenticated) {
      return;
    }
  ...
  const activeRoleData = ROLES[isAuthenticated ? currentRole : selectedRole];
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 flex flex-col justify-between">
      ...
      <header className="p-4 sm:p-6 flex items-center justify-between max-w-7xl w-full mx-auto">
        ...
        <div className="flex items-center gap-3">
          ...
          <Button
            ...
            onClick={() => navigate(isAuthenticated ? `/dashboard/${currentRole}` : '/')}
          >
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8 flex flex-col justify-center">
        {/* Authenticated Role Lock Notice */}
        {isAuthenticated ? (
          <div className="mb-8 p-5 rounded-2xl bg-indigo-50 dark:bg-indigo-950/50 border border-indigo-200 dark:border-indigo-800 flex flex-col sm:flex-row items-center justify-between gap-4">
            <div className="flex items-center gap-3.5">
              ...
              <div>
                <div className="flex items-center gap-2">
                  ...
                  <Badge variant="primary" size="sm">{ROLES[currentRole]?.title}</Badge>
                </div>
                <p className="text-xs text-slate-600 dark:text-slate-300 mt-1">
                  You are currently authenticated as <strong>{user.name}</strong> ({ROLES[currentRole]?.title}). To access a different ecosystem role, sign out first.
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2.5 w-full sm:w-auto shrink-0">
              <Button
                ...
                className="w-full sm:w-auto text-rose-600 hover:bg-rose-50 dark:hover:bg-rose-950/40"
              >
                Sign Out
              </Button>
              <Button
          ...
          /* SIH Showcase Fast Track Banner for unauthenticated visitors */
        ...
        <div className="text-center max-w-3xl mx-auto mb-10">
          ...
          <h1 className="text-3xl sm:text-4xl font-bold tracking-tight text-slate-900 dark:text-slate-100">
            {isAuthenticated ? 'Your Authenticated Role' : 'Choose Your Ecosystem Role'}
          </h1>
          <p className="text-slate-600 dark:text-slate-400 mt-2 text-sm sm:text-base">
            {isAuthenticated
              ? `Your active session is authorized strictly for the ${ROLES[currentRole]?.title} workspace.`
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5 mb-8">
          {roleKeys.map((key) => {
            ...
            const isUserRole = isAuthenticated && currentRole === key;
            const isSelected = isAuthenticated ? isUserRole : selectedRole === key;
            const isLocked = isAuthenticated && !isUserRole;

            return (
              <Card
                key={key}
                onClick={() => !isLocked && handleSelectRole(key)}
                variant={isSelected ? 'elevated' : 'interactive'}
                className={`p-5 flex flex-col justify-between relative transition-all duration-200 ${
                  isLocked
                    ? 'opacity-40 grayscale-[40%] cursor-not-allowed border-slate-200 dark:border-slate-800'
                    : isSelected
                    ? 'ring-2 ring-indigo-600 border-indigo-600 dark:border-indigo-500 dark:ring-indigo-500 bg-white dark:bg-slate-900 cursor-pointer'
                    : 'border-slate-200/80 dark:border-slate-800 cursor-pointer'
                }`}
              >
                <div>
                  <div className="flex items-center justify-between mb-4">
                    ...
                    {isSelected ? (
                      <span className="px-2.5 py-1 rounded-full bg-indigo-600 text-white text-xs font-semibold flex items-center gap-1 shadow-xs">
                        <CheckCircle2 className="w-3.5 h-3.5" /> {isAuthenticated ? 'Your Role' : 'Selected'}
                      </span>
                    ) : isLocked ? (
                      <span className="px-2 py-0.5 rounded-full bg-slate-200 dark:bg-slate-800 text-slate-500 text-[10px] font-semibold flex items-center gap-1">
              ...
              <div>
                <div className="flex items-center gap-2">
                  <span className="text-xs text-slate-400 font-semibold uppercase tracking-wider">
                    {isAuthenticated ? 'Active Workspace:' : 'Ready to enter:'}
                  </span>
                ...
                <h4 className="text-base font-bold text-slate-900 dark:text-slate-100 mt-0.5">
                  {isAuthenticated
                    ? `Access ${activeRoleData.title} Workspace`
