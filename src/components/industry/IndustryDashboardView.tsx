...
export const IndustryDashboardView: React.FC<IndustryDashboardViewProps> = ({
  ...
  const getTabFromPath = (): string => {
    ...
    if (path.includes('/candidates') || path.includes('/talent')) return 'candidates';
    if (path.includes('/jobs')) return 'jobs';
    if (path.includes('/applications') || path.includes('/pipeline')) return 'applications';
    if (path.includes('/cohorts') || path.includes('/internships')) return 'cohorts';
    if (path.includes('/learning-programs') || path.includes('/learning')) return 'learning_programs';
    if (path.includes('/interventions')) return 'interventions';
    if (path.includes('/projects') || path.includes('/challenges') || path.includes('/workshops')) return 'projects_workshops';
  ...
  const handleTabChange = (tabId: string) => {
    ...
    } else if (tabId === 'projects_workshops') {
      navigate('/dashboard/industry/projects');
    } else if (tabId === 'learning_programs') {
      navigate('/dashboard/industry/learning-programs');
    } else {
      navigate(`/dashboard/industry/${tabId}`);
    }
  };
