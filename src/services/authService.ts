...
export function parseProfileFirestoreError(error: any): { message: string; code: string } {
  ...
  if (isDev) {
    if (isOfflineOrTimeout) {
      console.warn('[SkillSetu Profile] Notice: Firestore offline or slow response; operating with workspace fallback', {
        code,
        message: rawMsg
      });
    } else if (code === 'permission-denied') {
      console.error('[SkillSetu Profile] Firestore Permission Denied:', {
        code,
        message: rawMsg,
        error
      });
    } else {
      console.warn('[SkillSetu Profile] Firestore notice:', {
        code,
        message: rawMsg
      });
    }
